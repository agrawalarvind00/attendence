...
def file(request):
    if request.method=='POST':
        ...
        for i in range(1,num+1):
            ...
            f = pd.read_csv('media/'+name, sep='\t', encoding='UTF-16')
        ...
        final.sort_values('1Name').sort_index(axis=1).to_csv('files/Attendance.csv',index=False)
        m = open('Files/Attendance.csv','r')
        data = m.readlines()
